...
async function run() {
    try {
        ...
        const downloadedPath = await toolcache.downloadTool(downloadUrl, outputPath, `token ${token}`, {
        ...
        const cachedDir = await toolcache.cacheDir(extractedPath, tool.name, version);
...
function toolPath(tool) {
    return path.join(getCacheDir(), tool.owner, tool.name, tool.version, `${tool.osPlatform}-${tool.osArch}`);
}
